...
| 22 | Los **años se cortan** en el efecto (número gigante recortado) | Timeline (Nosotros) | Bug visual | **Hecho (verif. Playwright)** | `leading-none`+`tracking-tighter` dentro de la ventana `overflow-hidden` recortaba el glifo (arriba/abajo y luego el último dígito a la derecha) → `px-[0.16em] py-[0.12em]` al span del año. Verificado: overflow 0 en los 4 lados, "2006" completo. |
...
| 25 | El **logo FIBERLUX** (escena Spline del hero) queda encima del searchbar | Home hero + buscador | Bug visual (compositing) | **Hecho (verif. Playwright)** | El "logo de FIBERLUX" ES la escena Spline (`heroBackground:"3d"`); su canvas WebGL (capa GPU con `translateZ(0)`) se colaba sobre el `backdrop-blur` del overlay en Chromium. Fix: evento `fbx:overlay` desde SearchOverlay → el hero pone la capa 3D en opacity 0 mientras el buscador está abierto. Verificado end-to-end: al abrir "Buscar", opacity de la capa = 0. |
