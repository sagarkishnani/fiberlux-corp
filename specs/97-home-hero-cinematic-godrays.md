...
- **Sí (revisado en QA):** se arrancó con **DOM/CSS + canvas 2D ligero**, pero el QA visual determinó que los god-rays CSS quedaban geométricos y planos, sin parecerse al ref. Se ejerció la puerta de escape prevista y **la escena de fondo (god-rays + haze + polvo) se reconstruyó en Three.js/WebGL**: shader de dispersión volumétrica (marcha hacia la luz sobre densidad de humo fbm) + partículas GPU additivas. Los **tokens** siguen en DOM (editables desde el CMS). `three` ya era dependencia (SPEC 96), así que no suma peso nuevo en el home. El cliente había aceptado esta escalada ("si no queda igual, prefiero Three.js/WebGL").
